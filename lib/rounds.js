...
import { rounds, progress, roundAttempts } from '../src/schema.js';
...
/** Sets (or clears, with null) the per-user attempt cap on the currently active round. */
export async function setMaxAttempts(roundId, max) {
  await db.update(rounds).set({ maxAttemptsPerUser: max }).where(eq(rounds.id, roundId)).run();
}

/** Total qualifying throws this user has made in this round so far, win or lose. */
export async function getAttemptCount(roundId, userId) {
  const row = db.select().from(roundAttempts)
    .where(and(eq(roundAttempts.roundId, roundId), eq(roundAttempts.userId, userId)))
    .get();
  return row?.count ?? 0;
}

/** Records one more qualifying throw (regardless of outcome) and returns the new total. */
export async function incrementAttempt(roundId, userId, userName) {
  const key = `${roundId}:${userId}`;
  const [row] = await db.insert(roundAttempts)
    .values({ roundId, userId, userName, count: 1, key })
    .onConflictDoUpdate({
      target: roundAttempts.key,
      set: { count: sql`${roundAttempts.count} + 1`, userName },
    })
    .returning()
    .all();
  return row.count;
